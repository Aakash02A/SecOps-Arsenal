...
def check_windows_baseline():
    ...
    try:
        res = subprocess.run('netsh advfirewall show currentprofile state', shell=True, capture_output=True, text=True)  # noqa: S602
        if "ON" in res.stdout:
    ...
    try:
        res = subprocess.run('net user Guest', shell=True, capture_output=True, text=True)  # noqa: S602
    ...
    try:
        ...
        res = subprocess.run(cmd, shell=True, capture_output=True, text=True)  # noqa: S602
...
def check_linux_baseline():
    ...
    try:
        res = subprocess.run('ufw status', shell=True, capture_output=True, text=True)  # noqa: S602
