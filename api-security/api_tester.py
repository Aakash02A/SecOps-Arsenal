...
def test_missing_auth(url, endpoint):
    ...
    try:
        ...
        if response.status_code in [200, 201]:
            print(
                f"  [!] VULNERABILITY FOUND: Endpoint is accessible without "
                f"authentication (HTTP {response.status_code})"
            )
        elif response.status_code in [401, 403]:
            print(
                f"  [+] Secure: Endpoint correctly rejected unauthenticated access (HTTP {response.status_code})"
            )
...
def test_bola(url, endpoint, token, user_id, target_id):
    ...
    try:
        ...
        if response.status_code in [200, 201]:
            print(
                f"  [!] VULNERABILITY FOUND: Successfully accessed resource belonging "
                f"to user {target_id}! (HTTP {response.status_code})"
            )
        elif response.status_code in [401, 403]:
            print(
                f"  [+] Secure: Access denied to other user's resource (HTTP {response.status_code})"
            )
        else:
            print(f"  [-] Unexpected response: HTTP {response.status_code}")
    except Exception as e:
    ...
    try:
        for i in range(requests_to_send):
            ...
            if response.status_code == 429:
                print(
                    f"  [+] Secure: Rate limit triggered at request #{i+1} (HTTP 429 Too Many Requests)"
                )
                hit_limit = True
                break
        ...
        if not hit_limit:
            print(
                f"  [!] VULNERABILITY FOUND: No rate limiting detected after {requests_to_send} rapid requests."
            )
    except Exception as e:
        print(f"  [-] Error: {e}")
...
def main():
    ...
    parser.add_argument(
        "url", help="Base URL of the API (e.g., https://api.example.com)"
    )
    parser.add_argument(
        "-e",
        "--endpoint",
        default="/api/users/{id}",
        help="Endpoint to test (default: /api/users/{id})",
    )
    parser.add_argument(
        "-t",
        "--token",
        default="MOCK_TOKEN",
        help="Valid auth token for a specific user",
    )
    parser.add_argument(
        "-u", "--userid", default="100", help="ID belonging to the token owner"
    )
    parser.add_argument(
        "-x",
        "--targetid",
        default="101",
        help="ID belonging to a different user (for BOLA test)",
    )
